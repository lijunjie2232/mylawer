...
export class EGovLawSearchPlugin extends BaseToolPlugin {
  readonly name = 'egov-law-search';
  readonly description = '日本の e-gov 法令検索 API を使用して法令を検索';
  ...
  get isEnabled(): boolean {
    // e-gov は API キー不要、常に有効（明示的に無効化されていない限り）
    const enabledVar = 'TOOL_EGOV_LAW_SEARCH_ENABLED';
  ...
  createTool(): DynamicTool {
    return new DynamicTool({
      ...
      description: `日本の e-gov 法令検索 API を使用して法令を検索し、最大${this.maxResults}件の結果を返します。
...
export const egovLawSearchFactory = createToolFactory(EGovLawSearchPlugin);
