...
_main() {
	...
	if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			# Create pg_cron extension and grant permissions using the already running temporary server
			printf 'Creating pg_cron extension and granting permissions...\n'
